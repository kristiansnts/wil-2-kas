...
export default async function Page() {
  ...
  return (
    <FormShell title="Catat Pengeluaran" back="/">
      <form action={addTxnUmumForm} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="form-group">
          ...
          <input className="form-input" type="text" name="desc" required placeholder="cth. Beli ATK" />
        </div>
        <div className="form-group">
          <label className="form-label">Tanggal</label>
          <input className="form-input" type="date" name="date" defaultValue={todayStr()} required />
        </div>
        <button type="submit" className="submit-btn" style={{ marginTop: 4 }}>Catat Pengeluaran</button>
      </form>
    </FormShell>
  )
}
